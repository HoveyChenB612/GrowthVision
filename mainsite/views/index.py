...
def index(request):
	...
	data_dict = {}
	for field in fields:
		queryset = models.PlatFormData.objects.filter(uid_id=uid).filter(platform=1).aggregate(Sum(field))
		data_dict.update(queryset)

	# 排名前十的作品
	top_10 = models.PlatFormData.objects.filter(uid_id=uid).annotate(
...
def index_total_zhihu(request):
	...
	data_dict = {}
	for field in fields:
		queryset = models.PlatFormData.objects.filter(uid_id=uid).filter(platform=2).aggregate(Sum(field))
		data_dict.update(queryset)
...
def index_total_bilibili(request):
	...
	data_dict = {}
	for field in fields:
		queryset = models.PlatFormData.objects.filter(uid_id=uid).filter(platform=4).aggregate(Sum(field))
		data_dict.update(queryset)
...
def index_total_baijiahao(request):
	...
	data_dict = {}
	for field in fields:
		queryset = models.PlatFormData.objects.filter(uid_id=uid).filter(platform=3).aggregate(Sum(field))
		data_dict.update(queryset)
...
def index_echarts(request):
	...
	for date in date_list:
		queryset = models.HistoryDate.objects.filter(date=date).values()
