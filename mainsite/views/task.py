...
def history_data_update():
	...
	for platform_uid in platform_uid_set:
		like_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			like_sum=Sum("like_count")).get("like_sum")
		comment_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			comment_sum=Sum("comment_count")).get("comment_sum")
		play_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			play_sum=Sum("play_count")).get("play_sum")
		download_rec_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			download_rec_sum=Sum("download_rec_count")).get("download_rec_sum")
		share_vote_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			share_vote_sum=Sum("share_vote_count")).get("share_vote_sum")
		forward_collect_sum = models.PlatFormData.objects.filter(platform_uid=platform_uid).aggregate(
			forward_collect_sum=Sum("forward_collect_count")).get("forward_collect_sum")

		nickname = models.PlatFormData.objects.filter(platform_uid=platform_uid).first().nickname
		platform = models.PlatFormData.objects.filter(platform_uid=platform_uid).first().platform
		if platform == 1:
			platform = "抖音"
		elif platform == 2:
			platform = "知乎"
			...
			platform = "哔哩哔哩"
		uid = models.PlatFormData.objects.filter(platform_uid=platform_uid).first().uid.uid
