$.fn.ECalendar = function (config) {
	...
	_this.click(function (event) {
		...
		if (height / 2 < top) {
			ECalendar.css({ left: post.left + config.offset[0], top: post.top - config.offset[1] - ECalendar.height() - 1 })
		} else {
	...
	ilast.click(function () {
		if (select.height() > 0) select.animate({ height: 0 }, 200)
		else select.animate({ height: 20 }, 200)
	})
	...
	$(imonth).mousedown(function (e) {
		...
		currenttime.animate({ opacity: 0.1 }, 600)
		$('body').addClass('ECalendarNoneSelect')
		_this.attr('readonly', 'readonly')
	})
	...
	function init() {
		...
		_this.after(
			'<svg t="1493098552635" class="icon" style="position: absolute;right:' +
				(_this.height() - 20) / 2 +
	...
	function setData(date) {
		...
		var val = config.format
			.replace('yyyy', getFullYear)
			.replace('mm', getMonth)
			.replace('dd', getDate)
